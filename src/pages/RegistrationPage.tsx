...
import { User, Building2, Mail, Lock, CheckCircle2, ArrowRight } from 'lucide-react'
...
export function RegistrationPage() {
  ...
  useEffect(() => {
    ...
    if (params.get('login') === 'true' || window.location.pathname === '/login') setIsLogin(true)
  }, [])
    ...
    <Container className="min-h-[80vh] flex items-center justify-center py-24 page-transition">
      <Card className="w-full max-w-lg p-12 border border-white/5 bg-card/50 backdrop-blur-xl shadow-2xl space-y-10 relative overflow-hidden rounded-[40px]">
        ...
        <button
          ...
          className="w-full h-14 flex items-center justify-center gap-3 border border-white/10 rounded-xl bg-white/5 hover:bg-white/10 transition-all font-bold tracking-widest text-[10px]"
        >
