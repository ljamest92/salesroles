...
// Create draft job
await db.execute(
  `INSERT INTO jobs (id, title, company_name, company_website, location, work_type, seniority, sector, description, base_salary, ote, currency, status, created_at)
   VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'draft', NOW())`,
  [jobId, 'Test Sales Role', 'Test Company', 'https://testcompany.com', 'London, UK', 'Remote', 'Mid-Level', 'B2B SaaS', 'This is a test job listing.', 80000, 120000, 'USD']
)
...
console.log('Job status:', (rows as any[])[0])
